...
@section('content')
    <div class="max-w-2xl mx-auto px-4 py-10 space-y-6">
        ...
        @forelse($colocations as $coloc)
            <div
                class="bg-white rounded-2xl border border-violet-100 shadow-sm overflow-hidden
                ...
                <div class="p-6 flex items-center gap-4">
                    ...
                    <div
                        class="w-12 h-12 rounded-xl bg-gradient-to-br from-violet-400 to-pink-400
                    ...
                    <div class="flex items-center gap-2 flex-shrink-0">
                        ...
                        @php
                            $role = $coloc->pivot->role ?? null;
                            $status = $coloc->pivot->status ?? null;
                        @endphp
                        ...
                        @if ($status === 'accepted')
                            ...
                            @else
                                <form action="{{ route('quittercolocation', $coloc->id) }}" method="POST">
                                    @csrf
                                    <button type="submit" onclick="return confirm('Quitter cette colocation ?')"
                                        class="flex items-center gap-1.5 px-4 py-2 rounded-xl border border-amber-200
                           text-amber-600 text-sm font-medium bg-amber-50
                           hover:bg-amber-100 hover:border-amber-300 active:scale-95
                           transition-all duration-200">
                                        Quitter
                                    </button>
                                </form>
        ...
        @empty
            ...
            <div class="bg-white rounded-2xl border border-violet-100 shadow-sm p-12 text-center">
                <div
                    class="w-16 h-16 rounded-2xl bg-violet-50 border border-violet-100
                        flex items-center justify-center mx-auto mb-4">
                    <svg class="w-8 h-8 text-violet-300" fill="none" stroke="currentColor" stroke-width="1.5"
                        viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round"
                            d="M2.25 12l8.954-8.955c.44-.439 1.152-.439 1.591 0L21.75 12M4.5 9.75v10.125c0 .621.504 1.125 1.125 1.125H9.75v-4.875c0-.621.504-1.125 1.125-1.125h2.25c.621 0 1.125.504 1.125 1.125V21h4.125c.621 0 1.125-.504 1.125-1.125V9.75M8.25 21h8.25" />
                    </svg>
